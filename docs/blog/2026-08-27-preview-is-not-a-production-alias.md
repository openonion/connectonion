...
local tests have a separate preview-specific override.
...
forces the paid path. Either paid outcome names the $0.025 interval price. The
...
The exact 1.8 browser candidate passed the focused preview and version suites,
the browser security/runtime matrix, the installed-wheel harness, and a real
ConnectOnion-to-Onionwright preview boundary check. A fresh install also proved
that the candidate ignored `OO_API_URL` and selected the preview origin without
reading credentials or touching the network in system mode.
...
source and wheel. When the consumer was rebased onto newer 1.8 hardening, the
source and wheel hashes necessarily changed, so the hosted gate became required
again for the rebased candidate.
