...
Status: active release execution; Linux PAYG path published and production-verified
Prepared: 2026-08-25; execution checkpoint updated 2026-09-01
Release authority: [connectonion#1154](https://github.com/openonion/connectonion/issues/1154)

## Artifact Stream checkpoint — 2026-09-01

The local BrowserDaemon foundation for [DD-062](design-decisions/062-oip-artifact-stream.md)
is ready for review. New local clients use one length-delimited OIP 0.2 Protocol
Buffers envelope for typed command argv, text results, errors, screenshot
metadata, 256 KiB data frames, completion, and receiver commit. Screenshots are
verified into caller-owned files; neither base64 nor a daemon-local path crosses
the new boundary. A multi-frame regression exceeds 1 MiB, demonstrating that
the per-message memory guard is no longer a file-size limit.

This checkpoint does **not** claim remote file transport. Wire v1 remains only
as a bounded migration reader for old local clients, and the OIP 0.2 artifact
writer is not enabled on Direct or Relay. Remote promotion waits for the reviewed
#1208 secure-channel protocol/library and deterministic conformance vectors.
Explicit Chromium download capture, durable resume, cancellation/flow control,
and remote carrier integration remain follow-up gates and must reuse this one
Artifact Stream rather than add `get`, URL, or base64 fallbacks.

## Execution checkpoint — 2026-08-27

The first vertical slice now has a production-verified Linux path. PRs remain
unmerged until release review; macOS is an enterprise-internal ad-hoc build,
not a public Developer-ID/notarized release.

Current immutable release evidence:

- `oo-api` production runs commit `a7f0139bc42423094ec23ecc80a1faf6abe6331b`.
  Its signed catalogue exposes only Chromium 151 Linux as paid-ready; download
  and preparation remain non-billable, while each start/renew interval is
  $0.025. The full local suite passes: 677 tests, with 154 expected skips.
- Onionwright `0.0.12` is published at
  `onionwright/0.0.12/onionwright-0.0.12-py3-none-any.whl`: 51,364 bytes,
  SHA-256 `506ba1efb9416646721148632e27229cd5c9bf3366e68918d540c22173759f47`.
  Its source is commit `b75cc1732b8a6356f8b82b3243db276aac3e4b9f`; all 273 tests pass.
  Version 0.0.12 fixes the production-discovered async disconnect/close race so
  `await close()` cannot return before its shared release completes.
- Chromium 151 Linux is published at
  `chrome/151.0.7922.137/linux-x86_64.tar.zst`: 211,322,904 bytes, SHA-256
  `7de5a8d9f76f9700ac4c731bd7b6532db21dfd892a7465e63d7fe825efd5570e`.
  Exact downloaded bytes passed native licence, renewal, CDP, HTTPS, WebGL,
  browser-download, and proxy-auth gates. A real production customer-path run
  charged exactly $0.025 and was already `released` when 0.0.12 close returned.
- The macOS arm64 build remains available only at the explicitly internal,
  noncanonical object
  `internal/chrome/151.0.7922.137/darwin-arm64-adhoc-f7d388b.tar.zst`, SHA-256
  `f43588dd4d970cef947bf3f36d2bebc2d9bf593a7baf0a248c10e9c10e03ee49`.
  It passes the native functional gate but is ad-hoc signed: it has no Apple
  Team ID, notarization ticket, staple, or public paid-catalogue eligibility.
  Developer ID signing/notarization is a later public-release gate.

Earlier implementation checkpoint (retained for design and review history):
...
- `oo-api` PR #183 implements the signed exact-platform manifest, non-billable
  ...
  recorded; 55 focused tests and the 672-test local suite pass with 154 expected skips.
  Review now also rejects extra or noncanonical paid-artifact metadata—including
  mismatched object keys, non-integer sizes, unsafe executable paths, and wrong package
  formats—before a catalogue can be signed or used for billing. New starts and renewals
  now construct and sign the runtime licence before transaction commit, so a minting
  failure rolls back the charge, session, and ledgers together.
  ...
  stream tokens; its review found and fixed cross-channel stream-token replay, carries
  the same canonical catalogue and before-commit minting gates, and 681 local tests
  pass. The disjoint 1.9 preview catalogue still contains no releasable browser
  artifact; the production 1.8 catalogue is described above.
- `onionwright` PRs #41–#43 implement typed preflight/acquisition, renewable session
  ownership, sync and cancellation-safe async paid launchers, and the private wheel. The wheel builds and
  installs locally, and its publisher is immutable and checksum-verifying. PR #43's
  paid-lifecycle review is recorded and all 271 tests pass in the combined oo-api
  dependency environment. A review fix now finishes an explicitly cancelled async
  close/release exactly once before propagating `CancelledError`. Head `b4180c3`
  preserves the exact reviewed wheel between
  credential-free preparation and upload, refuses dirty source for every release mode,
  verifies the preserved checksum and exact schema, and requires evidence to match the
  current source commit/tree. This paragraph describes the pre-publication
  checkpoint; the authoritative 0.0.12 production evidence is recorded above.
...
- `browser` PRs #104–#105 advance the paid target to Chromium 151.0.7922.137 and
  ...
  upload runtime gates, and a Python 3.10–3.12 matrix. Follow-up release review found
  that the WebGL probe printed failures but always exited zero and that the documented
  project-owned macOS E2E command did not exist. Head `b42baff` now makes the WebGL
  contract executable, defaults release handoff to a fixed native gate covering licence
  rejection plus renewal/expiry, packaged launch, CDP, WebGL, raw Runtime behavior, and
  native proxy auth, and adds credential-free unsigned bundle provenance preflight with
  a non-overwriting evidence directory, and binds GN generation, runtime-dependency
  packaging, and macOS inspection to the exact Chromium checkout rather than an
  unrelated executable on `PATH`. Linux `--prepare-only` now preserves the exact tested
  archive, checksum, runtime and patch manifests, GN args, and source identities;
  `--publish-prepared` requires the exact clean Browser commit/tree, validates those
  same bytes before any cloud call, and repeats the runtime gates before upload and
  after redownload without rebuilding. The macOS path likewise requires clean source,
  records its commit/tree, gates staging before promotion, and repeats the complete
  native E2E after downloading the final production key. Its unsigned preflight also
  requires the output below the claimed Chromium checkout and rejects generator
  depfiles naming another checkout, closing the stale-generated-file failure exposed
  by the relocated local build tree. All 55 tests plus 31 patch subtests pass. The exact
  six-patch source and manifest verify
  while a cold local macOS arm64 app build continues. Full application linking, native
  behavioral coverage, and certified Mac/Linux artifacts remain unproven while hosted
  builders are blocked before executing any job step by organization billing.
- `connectonion` draft PR #1280 is now stacked on the final async Browser Core/facade
  ...
  remains Draft pending final release review; Onionwright 0.0.12 and the Linux
  paid-ready artifact now exist.
- The separate `feat/1.8-remote-browser-paid-integration` branch composes #1307's
  ...
  It is draft PR #1308: mergeable against its stacked base. The current local evidence
  is 70 focused paid/native-egress tests and 7,621 broad tests passing, with 21 expected
  skips and 200 intentionally deselected tests. All 23 hosted checks passed on the prior
  code-bearing head across Python 3.10–3.13 and Linux/macOS/Windows; the latest docs-only
  head has no replacement check suite, so those historical checks are not presented as
  current-head CI. Its paid-boundary review is recorded and found no unresolved code
  issue; it remains draft for the native artifact and production-catalogue gates.
...
The releasable target prepared on this branch is `1.8.0a2`, not an artificial
merge of individually green components. Its entry gate is one exact, immutable artifact flowing through oo-api,
Onionwright, and ConnectOnion after redownload, with charge/renew/release evidence and
no fallback after billing. Linux is the public paid-ready platform. macOS arm64
stays an enterprise-internal ad-hoc build until Developer ID signing and
notarization make it eligible for the public paid catalogue. Remote
Browser/OIP/proxy work continues without weakening that artifact gate.
...
> path or an exact paid WTFbrowser artifact without hidden fallback, plaintext Relay
...
- Required `linux-x86_64` WTFbrowser artifact and exactly one verified Mac
...
2. Port driver calls to `onionwright.async_api`; keep timeouts and errors typed.
...
7. Own one pinned Playwright-compatible driver boundary and test the exact async/sync
   API consumed by ConnectOnion.

### 1C. WTFbrowser artifact pipeline
...
now gates navigation: the paid WTFbrowser must supply the daemon credential
...
```text
wtf    ─► Onionwright preflight ─► ready/funded ─► paid WTFbrowser session
                              └► typed failure; no fallback; charge $0 before start

chrome ─► Onionwright driver API + system Chrome/pinned Chromium; charge $0
         └► warn about bot detection and account risk on every command
```

Requirements:

- Both engines use Onionwright's one pinned driver API. Chrome remains independent
  of oo-api, licensing, and paid downloads.
- Preflight finishes local artifact readiness before session creation and billing.
- The selected engine and typed reason appear in status, JSON output, OIP, and audit.
- No hot swap exists. A failed/expired WTFbrowser session ends; Chrome starts only
  after an explicit CLI/config/API choice and uses its separate profile.
...
| WTFbrowser | Chromium revision, browser commit, patch/GN provenance, exact platform tags, URLs, hashes, minimum OS, signing/notarization |
