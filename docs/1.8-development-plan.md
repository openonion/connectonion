...
- `oo-api` PR #183 implements the signed exact-platform manifest, non-billable
  ...
  pass. Neither catalogue
  contains a releasable browser artifact yet.
- `onionwright` PRs #41–#43 implement typed preflight/acquisition, renewable session
  ...
  paid-lifecycle review is recorded and all 270 tests pass in the combined oo-api
  dependency environment. Head `329c324` now preserves the exact reviewed wheel between
  ...
  current source commit/tree; the verified 0.0.11 wheel is 51,165 bytes with
  SHA-256 `a4eae92c028add43dcdb964a93a174f8b13ce67e9a6e8a3231cc23428b155957`.
  Its bytes are identical to the earlier packet, but its evidence is now bound to the
  reviewed head rather than accepting a merely well-formed historical commit.
...
- `browser` PRs #104–#105 advance the paid target to Chromium 151.0.7922.137 and
  ...
  project-owned macOS E2E command did not exist. Head `4574ff5` now makes the WebGL
  ...
  native E2E after downloading the final production key. All 53 tests plus 31 patch
  subtests pass. The exact six-patch source and manifest verify
