...
**Rule:** Only inject messages in `@after_tool_round`, never in `@after_each_tool`.
...
| `@before_tool_round` | Before first tool in round | Once per round | Yes |
| `@before_each_tool` | Before each tool | Per tool | No* |
| `@after_each_tool` | After each tool | Per tool | No |
| `@after_tool_round` | After all tools in round | Once per round | Yes |
