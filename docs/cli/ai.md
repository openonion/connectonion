...
Default maps to Claude's manual permission mode, Auto-approve maps to
`acceptEdits`, and explicit Full access (YOLO) maps to Claude Auto mode. The
...
the `co ai` UI: Default can run actions allowed by its bound provider mode,
while other protected actions fail closed. Auto-approve automatically
