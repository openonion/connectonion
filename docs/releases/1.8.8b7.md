...
![co --help marks the new commands experimental](assets/v1.8.8b7/help-marks-experimental.png)
...
live bot. See [docs/cli/discord.md](../cli/discord.md) and
[docs/cli/telegram.md](../cli/telegram.md).
...
could not verify. Experimental. See [docs/cli/tiktok.md](../cli/tiktok.md).
...
[docs/useful_tools/generate_image.md](../useful_tools/generate_image.md).
...
```sh
python -m pip install --upgrade 'connectonion==1.8.8b7'
co --version
co benchmark --help
```

These notes first put `--pre` in that line. `--pre` reaches every
dependency, not only connectonion: it installed httpx 1.0.dev6, pydantic
2.14.0b2 and lxml 7.0.0b1, and httpx 1.0 has no `AsyncClient`, so `connect()`,
hosted agents and `co outlook` crashed. If you installed that way, run the
line above again without `--pre`; the next 1.8.8 preview also caps httpx below 1.
