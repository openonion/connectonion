...
| "End-to-end encrypted" | True for every OIP WebSocket session between 1.8.1 peers — direct since 1.8.0, through the relay since 1.8.1: `SEAL`/`SEALED` in `network/sealed.py`, one-time X25519 keys signed by both Ed25519 identities, every frame in a NaCl Box; the relay forwards ciphertext it cannot open. Still plaintext to the relay when either side is 1.8.0 or older, and the signed HTTP routes (`/info`, admin, contacts) are TLS only. Say "sessions are end-to-end encrypted", not "every byte that touches oo-api is". |
