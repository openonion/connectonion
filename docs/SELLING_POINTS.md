...
| "End-to-end encrypted" | True for **direct** sockets since 1.8.0b2: `SEAL`/`SEALED` in `network/sealed.py`, one-time X25519 keys signed by both Ed25519 identities, every frame in a NaCl Box. Still false for the relay path: TLS to a relay that terminates it — encrypted in transit, plaintext to the relay. Say "direct connections are end-to-end encrypted", not "everything is". |
