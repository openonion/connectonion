...
| First start consents once, installs the clock, runs one batch; repeat start asks nothing and reruns nothing while the summary is unchanged, and asks again when it changed (`test_start_asks_again_when_what_the_summary_shows_has_changed`) | `test_first_start_consents_installs_and_runs_one_batch_then_repeat_start_does_not_rerun`, `test_start_yes_then_stop` | Passing with an injected scheduler and runner |
