...
LLM-Note:
  ...
  Data flow: Agent.serve() → connect(relay_url) → WebSocket established → send_announce(ws, announce_msg) → serve_loop() → wait_for_task(ws) receives INPUT from relay → task_handler(prompt) executes → send OUTPUT response → heartbeat re-announces every 60s
  ...
  Integration: exposes connect(relay_url), send_announce(ws, msg), wait_for_task(ws, timeout), serve_loop(ws, announce_msg, task_handler, heartbeat_interval) | used by Agent.serve() to make agent discoverable on relay network | task_handler is async function (prompt: str) -> str
