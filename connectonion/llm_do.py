...
LLM-Note:
  Dependencies: imports from [typing, pathlib, pydantic, dotenv, prompts.py, llm.py] | imported by [debug_explainer/explain_context.py, user code, examples] | tested by [tests/e2e/real_api/test_llm_do.py, no dedicated file — exercised indirectly by tests/unit/test_reflect_handler.py and tests/unit/test_intent_detection_is_not_load_bearing.py]
