...
LLM-Note:
  ...
  Data flow: after_llm event fires → check_and_compact() checks context_percent → if >= 90% and len(messages) >= 8: _do_compact() → llm_do() with the product default summarizes old messages → replaces old messages with summary → keeps system + summary + last 5 messages → returns "{old_count} → {new_count} messages"
  ...
  Performance: only fires when context >= 90% | minimum 8 messages required | keeps last 5 messages (recent_count) + system + summary | llm_do() call to the product default (fast/cheap) | summary prompt limited to 800 words
