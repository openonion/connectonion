...
class Agent:
    ...
    def _drain_interjections(self) -> None:
        ...
        if not self.io or not hasattr(self.io, 'pop_interjections'):
            return
        for msg in self.io.pop_interjections():
            prompt = msg.get('prompt')
            if not prompt:
                continue
            framed = INTERJECTION_FRAME_PREFIX + prompt
            self.current_session['messages'].append({"role": "user", "content": framed})
            self._record_trace({
                'type': 'user_input',
                'id': msg.get('id'),
                'content': prompt,
                'turn': self.current_session.get('turn', 0),
