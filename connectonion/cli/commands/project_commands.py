...
def get_template_info() -> list:
    ...
    return [
        ('minimal', '📦 Minimal', 'Basic agent'),
        ('web-research', '🔍 Web Research', 'Web scraping'),
        ('email-agent', '📧 Email Agent', 'Email assistant'),
        ('meta-agent', '🤖 Meta-Agent', 'Dev assistant'),
...
def api_key_setup_menu(temp_project_dir: Optional[Path] = None) -> Tuple[str, str, Path]:
    ...
    try:
        ...
        if choice == 1:
            ...
            if api_key:
                provider, key_type = detect_api_provider(api_key)
                click.echo(f"{Colors.GREEN}✓ {provider.title()} API key configured{Colors.END}")
            ...
            click.echo(f"\n{Colors.CYAN}⭐ Get 100k Free Tokens{Colors.END}")
            click.echo("\nOpening GitHub in your browser...")
            ...
            while True:
                ...
                if already_starred:
                    click.echo(f"{Colors.GREEN}✓ Thank you for your support!{Colors.END}")
                    click.echo(f"\n{Colors.YELLOW}Authenticating to activate your free credits...{Colors.END}\n")
                    
                    try:
                        do_auth_flow()
                        click.echo(f"\n{Colors.GREEN}✓ We verified your star. Thanks for supporting us!{Colors.END}")
                        ...
                        click.echo("  • co/gpt-4o-mini")
                        click.echo("  • co/gpt-4o")
                        click.echo("  • co/o4-mini")
                        click.echo("  • co/gpt-5")
                        click.echo("  • co/claude-3-haiku")
                        click.echo("  • co/claude-3-sonnet")
...
def configure_env_for_provider(provider: str, api_key: str) -> str:
    ...
    configs = {
        ...
        'connectonion': {
            ...
            'model': 'co/gpt-4o-mini'  # Prefixed models for managed keys
        }
    }
...
# Available models: co/gpt-4o-mini, co/gpt-4o, co/claude-3-haiku, co/claude-3-sonnet
    ...
    if files_created:
        ...
        for file in files_created:
            if file == "agent.py":
                ...
                click.echo(f"  • {Colors.GREEN}{file}{Colors.END} - Environment configuration (add your API key)")
            elif file == "README.md":
...
def handle_create(name: Optional[str], ai: Optional[bool], key: Optional[str], 
        ...
        if temp_project_dir and temp_project_dir.exists():
            try:
                if click.confirm(f"\n{Colors.YELLOW}Remove temporary project directory '{temp_project_dir}'?{Colors.END}", default=True):
                    ...
                    click.echo(f"{Colors.GREEN}✓ Temporary directory removed.{Colors.END}")
            except Exception:
                pass
        sys.exit(0)
    ...
    if template == 'custom':
        ...
        click.echo(f"{Colors.GREEN}✓ Generated custom agent code{Colors.END}")
        click.echo(f"{Colors.GREEN}✓ Suggested project name: {ai_suggested_name}{Colors.END}")
        ...
        if template == 'custom':
            ...
            if ai_suggested_name:
                ...
                try:
                    ...
                    click.echo(f"{Colors.GREEN}✓ Project name:{Colors.END} {name}")
    ...
    if temp_project_dir:
        ...
        if project_dir.exists():
            click.echo(f"{Colors.RED}❌ Directory '{name}' already exists!{Colors.END}")
