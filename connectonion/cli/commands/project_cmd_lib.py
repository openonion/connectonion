...
LLM-Note:
  ...
  Errors: validate_project_name() returns (False, error_msg) for invalid names | detect_api_provider() returns ("openai", "unknown") for unrecognized keys | generate_custom_template_with_name() may fail if LLM API unreachable | api_key_setup_menu() catches KeyboardInterrupt and returns ("", "", None) | no try-except blocks (follows fail-fast principle)
"""
