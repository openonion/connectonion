...
def get_template_content(template_name: str, file_type: str) -> str:
    ...
    template_map = {
        ...
        ("meta-agent", "metagent"): "metagent.md",
        ("meta-agent", "docs_retrieve"): "docs_retrieve_prompt.md",
        ("meta-agent", "answer"): "answer_prompt.md",
        ("meta-agent", "think"): "think_prompt.md",
        ("meta-agent", "readme"): "README.md",
        ("basic", "agent"): "meta_agent.py",  # Alias for backward compatibility
        ("basic", "prompt"): "metagent.md",  # Alias for backward compatibility
...
def init(template: str, with_examples: bool, force: bool):
    ...
    try:
        ...
        # For meta-agent template, create prompts folder structure
        if template == "meta-agent":
            ...
            prompt_files = {
                "metagent": "metagent.md",
                "docs_retrieve": "docs_retrieve_prompt.md",
                "answer": "answer_prompt.md",
                "think": "think_prompt.md"
            }
            
            for prompt_type, filename in prompt_files.items():
                prompt_content = get_template_content(template, prompt_type)
                prompt_path = os.path.join(prompts_dir, filename)
                
                if create_file_if_not_exists(prompt_path, prompt_content, force):
                    files_created.append(f"prompts/{filename}")
                else:
                    files_skipped.append(f"prompts/{filename} (already exists)")
        else:
            # Create single prompt.md for other templates
        ...
        # Create README.md for meta-agent template
        if template == "meta-agent":
            readme_content = get_template_content(template, "readme")
            readme_path = os.path.join(current_dir, "README.md")
            
            if create_file_if_not_exists(readme_path, readme_content, force):
                files_created.append("README.md")
            else:
                files_skipped.append("README.md (already exists)")
