...
It is local workflow state; OIP permission profiles remain owned by the Host
transaction rather than WebSocket mode-change frames.
...
def exit_plan_and_implement(agent=None) -> str:
    ...
    # user's Read only/Auto/Full access OIP permission profile.
    if agent:
