"""Tools for watches created by the Agent in its current Host session."""
...
def _context(agent):
    ...
    if store is None or not session.get("session_id"):
        raise RuntimeError("Watches require a running co ai Host session")
    if requester.get("level") != "admin" or not requester.get("address"):
        raise PermissionError("Only the Host owner can create or manage watches")
...
def watch_every(minutes: int, probe: str, query: str = "",
    ...
    from ....network.host.session.watches import gmail_probe
