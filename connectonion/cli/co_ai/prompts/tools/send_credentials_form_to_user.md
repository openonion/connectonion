...
- Saves the submitted credentials for the current login flow without returning their raw values.
...
- After this tool returns, focus the relevant input fields and call `type_saved_login_credential` to fill the current login page in the same turn.
...
- After success, failure, or abandonment, close the browser before your final response.
