...
def test_python_decoder_binds_mode_to_the_active_session():
    assert legacy_stream_event_from_acp(
        mode_frame("auto_approve"), expected_session_id=SESSION_ID
    ) == {"type": "mode_changed", "mode": "auto_approve"}

    with pytest.raises(ValueError, match="another session"):
        legacy_stream_event_from_acp(
            mode_frame("full_access", "other-session"),
...
def test_python_remote_agent_deduplicates_acp_and_legacy_mode_output():
    ...
    agent._handle_stream_event(mode_frame("full_access"))
...
def test_python_remote_agent_ignores_unowned_or_unknown_modes(event):
    agent = RemoteAgent("0xabc")
    agent._current_session = {"session_id": SESSION_ID, "mode": "default"}
...
async def test_host_dual_writes_mode_in_order():
    ...
    io.send({"type": "mode_changed", "mode": "auto_approve"})
    io.mark_agent_done()
    ...
    assert sent[0] == mode_frame("auto_approve")
    assert sent[1]["type"] == "mode_changed"
    assert sent[1]["mode"] == "auto_approve"
