...
import sys
import os
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '../../connectonion/useful_tools'))
...
def test_search_auth_failure_with_fallback(mock_httpx):
    ...
    mock_response_auth.raise_for_status.side_effect = Exception("401 Unauthorized")
...
def test_get_contents_auth_required(mock_httpx):
    ...
    mock_httpx.post.return_value = mock_response
    mock_response.raise_for_status.side_effect = HTTPStatusError("401", request=Mock(), response=mock_response)
