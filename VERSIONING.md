...
## Current Version: 1.7.0rc8

### Version History
- 1.7.0rc8 (**a persistent browser no longer pays to import its entire implementation on every command.** RC7's exact live gate completed real browser work and compiled-language agents, but repeated state queries and cleanup appeared to wedge behind the daemon. An absent-daemon `co browser status` reproduced the real boundary: the client spent 40.82 seconds importing Agent, Playwright, TUI, and provider integrations before it ever reached the socket. Direct browser RPCs now load only their transport path; help and natural-language `do` retain their full schemas through lazy imports. The same cold status check takes 2.91 seconds, while a real isolated-browser smoke completes warm page-state queries and per-tab cleanup in 4.50 and 4.44 seconds. This is an opt-in RC8 testing candidate, not Stable/Latest; the exact public bytes must repeat the complete gate before unchanged promotion. Slow whole-browser graceful teardown is isolated to the bounded cleanup path and tracked for 1.7.1 in #1250.)
