...
1.8.9b7 is an opt-in preview for subscribed skills and browser task syntax
...
1.8.8. See [1.8.9b7 notes](docs/releases/1.8.9b7.md).

- 1.8.9b7 (subscription reconciliation and signed companion files; browser task syntax without `do`.)

Earlier in this line:

## Previous preview: 1.8.9b6
...
- 1.8.0b1 (**the first 1.8 beta: the Remote Browser lends your computer's connection across the internet, and the paid engine can pay.** `co proxy share` now dials out from your computer to the host, so it works from behind any home or office NAT — nothing listens on your side, and the host pushes resolve/connect/data down one authenticated WebSocket. `co remote-browser config` remembers the host once, headed is the default, and the `remote-browser` session state survives host restarts. Onionwright 0.0.13 is required and installed: it is the first client that signs the spend authorization production has demanded since the server-side change, and a final version defaults to the production channel. Measured 2026-09-02 across two machines — a Google Cloud server (`35.229.135.74`) and a laptop behind a home NAT in Sydney: `api.ipify.org` and `ifconfig.me` both saw the laptop's `129.94.43.159`; `co proxy stop` broke the tab with `ERR_TUNNEL_CONNECTION_FAILED` instead of falling back; the session cost exactly one interval, $0.025. Also carried: global-credential `co init` by default, dotenv loaded from the canonical project root, secure SMS pairing, the canonical `oo` skill, OIP relay-session fixes. Beta means the 1.8 feature set is frozen for exercise; `1.8.0` is cut once this path has been run by more than its authors.)
- 1.8.0a5 (**a phone joins an Agent's SMS inbox without ever receiving the Agent private key.** `co sms pair` creates a short-lived Ed25519-signed QR challenge; Android verifies it, creates a non-exportable P-256 Keystore identity, and signs its claim. The owner compares a six-digit short authentication string before the Agent signs activation for that exact device key. The resulting phone credential is upload-only and independently revocable, pending pairing survives Android restarts, and the server stores hashes, public keys, signatures, and ciphertext rather than either endpoint's private key. The Python APIs and CLI cover pairing, inbox reads, acknowledgement, deletion, device inventory, and revocation; v1 remains readable during rollout. Shared Python/Kotlin vectors and a hosted Android → oo-api → PostgreSQL → Agent lifecycle prove signed pairing, decryption, and synchronized deletion. The six digits are a human comparison aid, not a standalone authenticator, and the protocol has not received an independent cryptographic audit.)
- 1.8.0a4 (**an AI agent can receive SMS without giving the server the message.** `create_sms_pairing()` binds an Android default-SMS endpoint to one Agent address; `get_sms()` and `wait_for_sms()` fetch ciphertext from oo-api and decrypt it only inside that Agent project. The tools support acknowledgement, deletion, device listing and revocation, validate every path identifier, and always mark SMS as untrusted input. The frozen cross-language vector proves Kotlin/Python sealed-box compatibility; tampered ciphertext fails closed.)
- 1.8.0a3 (**a browser on a server can now reach the internet through your own computer.** `co proxy share to <address>` lends this machine's connection to one authorized agent, and `co remote-browser <address> start --proxy shared` sends that session's egress through it — so pages see your address instead of the data centre's. Measured across two machines: a Google Cloud server running the paid Chromium 151 reported `129.94.43.159`, the sharing laptop's address, rather than its own `34.21.243.229`. Sharing a connection is not sharing the network behind it: the share applies the same destination policy as the host's own gateway, so the sharer's router, localhost and LAN answer `403 DESTINATION_ADDRESS_DENIED`. The host still resolves and pins one numeric address itself and asks the share for that exact address, never the hostname.

  This rests on the fail-closed egress boundary that lands in the same release: a frozen destination policy, an authenticated loopback gateway that owns DNS and dials only approved numeric sockets, a Host-private browser runtime separate from local `co browser`, and a native preflight that makes the browser prove it used the gateway before any page is opened. Each level was reviewed adversarially before merge; six real defects were found and fixed, including a WebRTC switch Chrome does not define that left non-proxied UDP fully open, and a preflight that could never pass while every test was green.

  the first end-to-end Linux paid Onion Browser preview.** Engine selection is resolved once as `auto`, `system`, or strict `onion` before a session can charge; system mode remains credential-free and non-billable, while paid preparation verifies the exact signed platform manifest, artifact checksum, compatibility, and executable before the $0.025 interval boundary. `co browser install-onion` now bootstraps the private Onionwright 0.0.12 wheel through an authenticated, pinned-Ed25519, checksum-verified production path without starting a daemon or paid session. The Linux Chromium 151 customer path has passed production download, launch, HTTPS, WebGL, browser-download, renewal, close/release, and native proxy-auth acceptance. Remote Browser pins the private engine and proves server-owned egress before seed cookies or a user page. macOS remains an explicitly internal ad-hoc enterprise build; Developer ID signing, notarization, and public paid-catalogue promotion are later gates.)
- 1.8.0a2 (**the async browser core goes concurrent**: the browser daemon runs sessions concurrently instead of one serial loop — one long `do` no longer starves every other session; the synchronous BrowserAutomation API is preserved unchanged over the async core, so existing scripts and skills keep working verbatim (#500); the Remote Browser gains an owner-bound lifecycle over OIP (#1298 content, start/status/stop settled deterministically); and the full async driver parity audit closes the last four public verbs (#1288). Cut from the exercised #1317 consolidation, hosted matrices green on Python 3.10–3.13, macOS and Windows.)
...
- 1.7.4 (**a deploy that deletes the replay ledger no longer takes a 1.7 host down with it.** A 1.7 host keeps one-use signature digests in `.co/replay.sqlite3` and created that schema once, at startup, so any deploy that replaced or removed the file left an empty database behind: every later signed call raised `no such table: used_signatures` and the host refused all authenticated work while still reporting itself healthy. That is what took the Melbourne rental host offline for 2h44m on 2026-09-03. The store now tells a vanished ledger from a real fault — only a missing table is rebuilt, once, with the claim retried once and a warning naming the file, the likely cause and the consequence, while corruption, `database is locked` and I/O errors keep failing closed. 1.8 fixes the same outage differently, by giving a single-process host an in-memory ledger, which needs the sealed channel 1.7 does not have. Backport #1420 of #1403; ledger #1407.)
- 1.7.3 (**global credentials by default, and one project keeps one environment.** Plain `co init` no longer writes project files: it creates or reuses the global identity and `~/.co/keys.env`, and project setup is `co init ./` or an explicit directory. Existing global provider keys are preserved; only an explicit `--key` is saved globally. Offline setup keeps the signing identity and recovers a missing `keys.env` without rotating the keypair. Project `.env` resolves from the same bounded project root as identity and auth, even from a subdirectory: explicit process variables win, then project `.env`, then global `~/.co/keys.env`. Carries the canonical `oo` useful skill, the corrected subscription hint, and the pypdf lockfile security update. No browser product changes. The same fixes are in main from 1.8.0b1. The earlier 1.7.2 candidate was cancelled before publication; its tag was not moved.)
- 1.7.2 (**one project keeps one environment, even from a subdirectory.** Package startup and Gmail, Outlook, GDrive, and Synology CLI reloads now resolve the same bounded project root as identity, auth, and status. Explicit process variables still win, project `.env` comes next, and global `~/.co/keys.env` remains a fallback; an unrelated nested `.env` no longer supplies credentials. The redundant CLI-startup dotenv read is removed. This patch also packages the canonical `oo` useful skill and corrected subscription hint from #1379, plus the pypdf 6.16.2 lockfile security update. No browser product changes. Product/test/docs forward-integration is tracked in #1380.)
- 1.7.1 (**a framework-correctness patch with no browser product changes.** OpenRouter `sk-or-` keys are detected before generic OpenAI keys; Outlook reuses usable Microsoft access tokens and distinguishes OpenOnion login from Microsoft re-consent; protected-file refusals carry truthful tool-failure status without overwriting the file; and max-iteration runs retain exit code 1 while reporting `✗ incomplete` instead of `✓ complete`. Each constituent fix passed the full Linux, Windows, and macOS matrices, including the unchanged real-browser regression gates.)
...
- 1.7.1 (**stable Agent outcomes and provider authentication fail honestly.** Max-iteration runs and refused file-tool work report incomplete/failed outcomes instead of success; Outlook reuses valid Microsoft tokens; and OpenRouter credentials are identified before OpenAI routing.)
