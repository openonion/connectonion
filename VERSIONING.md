...
- 1.5.18 (**published email/security patch and an exercised release chain**: carries the safe idempotent email retry baseline, dependency security floors and release hardening; fixes artifact acceptance so it force-installs the wheel under test instead of silently reusing an installed source copy; the exact CI wheel and sdist were uploaded to PyPI and verified byte-for-byte against the public files)
- 1.5.17 (**tag-only release rehearsal; not published to PyPI**: replaced the nonexistent root `requirements.txt` install with the package's declared dev dependencies, then stopped safely when wheel acceptance discovered that the outer source install could mask the wheel under test)
- 1.5.16 (**tag-only release rehearsal; not published to PyPI**: all reusable test jobs passed, then build stopped safely before PyPI because the workflow referenced a nonexistent root `requirements.txt`)
- 1.5.15 (**tag-only release rehearsal; not published to PyPI**: the tag job installed pytest and the package's runtime dependencies before running wheel acceptance; 1.5.14 had stopped safely when that runner dependency was missing)
- 1.5.14 (**tag-only release rehearsal; not published to PyPI**: safe retries for email and a security-hardening test release; every send carries a traceable request id and tenant-scoped idempotency key through Connect, oo-api and Resend, provider failures return stable JSON, relay profiles and remote control frames are authenticated more strictly, and dependency floors are raised; the automated publish attempt stopped before PyPI because the artifact job itself had not installed pytest)
