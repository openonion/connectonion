...
- 1.6.4 (**Gmail HTML bodies are parsed defensively**: a message whose HTML shape differs from the parser's expectation no longer raises and breaks inbox reading.) Published as [GitHub release `v1.6.4`](https://github.com/openonion/connectonion/releases/tag/v1.6.4).
- 1.6.3 (**status and billed boundaries use the account they name**: `co status` reports the identity commands actually use and the source of each credential; a billed operation refuses an ambient key that belongs to a different project account.) Published as [GitHub release `v1.6.3`](https://github.com/openonion/connectonion/releases/tag/v1.6.3).
- 1.6.2 (**an agent reads its own project trust list**: project discovery stops at the repository and home boundaries instead of treating global `~/.co` state as a project's configuration.) Published as [GitHub release `v1.6.2`](https://github.com/openonion/connectonion/releases/tag/v1.6.2).
- 1.6.1 (**the CLI stops acting as the wrong account**: a project-local credential cannot silently shadow this machine's identity; Outlook HTML reads retain link destinations; malformed JWT claims fail cleanly instead of crashing every command.) Published as [GitHub release `v1.6.1`](https://github.com/openonion/connectonion/releases/tag/v1.6.1).
