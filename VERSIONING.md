...
## Current Version: 1.7.0a3

### Version History
- 1.7.0a3 (**the coding-agent cards go live**: the preview pair that makes O Chat's live Codex/Claude Code cards work — this package ships the `coding_agents` plugin (`CodexPlugin`/`ClaudeCodePlugin`, first tagged in a2's tree but never included in a published artifact) and the `no-store` discovery header (#996) that a1 lacked, so oo-chat connects. Pairs with `@connectonion/react` 0.4.2-alpha.3. Direction note: OIP is now the canonical protocol (#998); ACP conformance work is shelved to ~2.x.)
