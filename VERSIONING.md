...
- 1.5.17 (**the email retry fix reaches PyPI through the package metadata that actually exists**: 1.5.16 passed the full cross-platform suite but its artifact job referenced a nonexistent `requirements.txt` and stopped safely before upload; this release installs the declared `.[dev]` dependencies and republishes the same safe-send fix under a new immutable version)
