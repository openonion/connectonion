...
### Version History
- 1.6.12 (**the stable line absorbs the proven work and stops hanging on a dead socket**: every LLM provider client now carries explicit network bounds — connect 20s, read 600s, bounded retries — so a server that closes its side mid-request ends in a typed LLMConnectionError instead of the CLOSE_WAIT hang that froze a scheduled run for 15+ minutes (#1116). `co server new --region` picks where a server is provisioned, the way out once one region's address quota fills (#713). `co email share`/`unshare` grant another account send access to one of your addresses without transferring it or sharing a key (#1137). `co outlook download` lands with collision-safe filenames (#923) and skips embedded signature images unless `--include-inline` asks for them (#924). The README repositions ConnectOnion as a template-first FDE toolkit (#1122); issue templates carry the AI implementation contract (#1123); and every release now records permanent visual evidence or an explicit reviewed exemption (#1124).)
- 1.6.11 (**a stable Control Center and one coherent execution-mode contract**: hosted agents expose a responsive Control Center with accessible light/dark tokens, capability filtering and diagnostics; new sessions default to versioned Auto Approve for reversible workspace work while Safe and Host-bounded Full access remain explicit; Plan stays a separate workflow mode; denied adapter discovery no longer prevents Host startup; Windows E2E produces faster, reproducible evidence; and the published React bridge acknowledges the exact Host wire mode. A real multi-round C introsort run found and fixed a release blocker where bounded `--yolo` announced Full access while the new policy still denied tools.)
- 1.6.10 (**Chinese, Japanese and Korean browser typing completes instead of crashing after its first character**: the humanized CJK paste path again measures the focused input or contenteditable before and after paste. A clipboard refactor had accidentally removed that small probe while leaving both callers behind, so `_active_text_len` raised `NameError` mid-entry and could leave a dangerous one-character fragment in a composer.)
...
- 1.6.6 (**the default model is Gemini 3.7 Flash, and the CLI stops hiding features**: every default — Agent, `llm_do`, `co ai`, transcribe — now uses `co/gemini-3.7-flash` (1M context, Google's newest Flash; 3.6 stays available as rollback and explicit configurations are untouched). `co transfer <address> <amount>` finally exposes the peer-to-peer credit transfers the backend has served for months, with an explicit confirmation before anything irreversible and a history listing. `co email addresses` shows every address an account owns and the send `--from` ownership error now names it as the next step. Next-step tips survive piping — they used to hide from scripts and AI callers, exactly the audience that needs them.)
- 1.6.5 (**a deployed agent runs on its own account, and the mailbox CLI behaves like a CLI**: three mail fixes ship alongside the deploy-identity fix. `co email` failures now exit 1 instead of 0, so a scripted `co email send … && next` no longer walks past a failed send — its gmail/outlook siblings always did this. `co email send --from` chooses which of the account's addresses to send as, verified server-side (an address you don't own is refused cleanly). The mail skill was rewritten against a new cli-skill-design methodology whose tips and error paths are tested, not asserted. And the original headline:  `co deploy --to` used to copy the project `.env` to the server verbatim, including the four identity lines `co init` writes so a project runs as its author locally. The deployed agent was therefore cryptographically itself and financially its author — it sent mail from the operator's mailbox and charged every model call to the operator's balance, with nothing failing and nothing to separate the spend afterwards. The deploy now withholds those keys and substitutes the agent's own, derived from the key the deploy already holds.)
