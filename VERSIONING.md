...
and must be forward-ported through every active preview. The paid-browser work
continues on the isolated 1.9 preview stacked on the reviewed 1.8 candidate.
Pre-releases are opt-in and must be marked as pre-releases on PyPI and GitHub.

## Current Version: 1.9.0a1

### Version History
- 1.9.0a1 (**the paid browser preview is a separate release channel, not a production alias.** ConnectOnion pins the exact Onionwright 0.0.13.dev2 bytes and `preview` runtime channel, uses only the dedicated `browser-preview.oo.openonion.ai` bootstrap origin (or an explicit ConnectOnion preview-only override), and rejects a correctly signed manifest that names any other channel before download or pip. System mode still returns before credentials, network, optional imports, downloads, or billing. Hosted preview acceptance remains gated on the isolated preview API deployment and a real exact-artifact $0.025 lifecycle; this candidate must not merge or publish merely because offline tests pass.)
- 1.8.0a2 (**the first end-to-end Linux paid Onion Browser preview.** Engine selection is resolved once as `auto`, `system`, or strict `onion` before a session can charge; system mode remains credential-free and non-billable, while paid preparation verifies the exact signed platform manifest, artifact checksum, compatibility, and executable before the $0.025 interval boundary. `co browser install-onion` now bootstraps the private Onionwright 0.0.12 wheel through an authenticated, pinned-Ed25519, checksum-verified production path without starting a daemon or paid session. The Linux Chromium 151 customer path has passed production download, launch, HTTPS, WebGL, browser-download, renewal, close/release, and native proxy-auth acceptance. Remote Browser pins the private engine and proves server-owned egress before seed cookies or a user page. macOS remains an explicitly internal ad-hoc enterprise build; Developer ID signing, notarization, and public paid-catalogue promotion are later gates.)
