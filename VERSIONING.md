...
### Version History
- 1.7.0a10 (**an equivalent live OIP reattach re-authenticates identity without re-authorizing the same connection**: after alpha 9 proved continuity, public fresh-session acceptance found that first Send could race another CONNECT through the still-live relay queue. Reattach now freshly verifies signature, recipient, replay claim, caller, capability, protocol, session, and current blacklist, but does not repeat mutable onboarding/contact policy already granted to that live connection. A missing local trust file can no longer prevent INPUT after a successful CONNECT.)
