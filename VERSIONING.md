...
### Version History
- 1.6.9 (**contacts can work, deploys preserve the identity and state they run with, and reads stop changing mail by surprise**: invited contacts can approve ordinary work in their own sessions while control-plane changes remain admin-only; the real `whitelist` level is honored; remote deploys authenticate the server-retained key, clear stale account metadata, and respect the project's ignore contract so laptop copies do not overwrite live runtime state; append and ordinary output redirects work without reopening shell file-authoring bypasses; `co email`, `co gmail`, and `co outlook` preserve unread state by default and expose `--mark-read`; Claude Code `allowed-tools` declarations now warn explicitly instead of silently losing their intended approval behavior; and the real installed-wheel Windows E2E keeps all coverage while removing redundant browser-daemon cycles.)
