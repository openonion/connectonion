...
- 1.5.16 (**the email retry fix reaches PyPI**: this is the first publicly installable build carrying the 1.5.14 safe-send candidate and the repaired 1.5.15 artifact acceptance runner; every send uses a traceable request id and tenant-scoped idempotency key so retrying an uncertain response cannot send the same message twice)
