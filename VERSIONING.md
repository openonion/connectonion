...
Stable users remain on the 1.7.x line. Higher release trains are opt-in until
they earn a stable release, and every pre-release must be marked as such on
PyPI and GitHub.

## Current Version: 1.7.3

### Version History
- 1.7.3 (**global credentials unless you name a project.** Plain `co init` initializes `~/.co/keys.env` and the machine identity without writing project files. Use `co init ./` or another explicit existing directory for project initialization; template, description, and overwrite options require that path. Global setup saves only explicitly supplied provider keys, preserves identity offline, and restores a missing keys.env without rotating the keypair. Also includes the project-root environment-loading fix, canonical `oo` skill, subscription hint, and pypdf lock update prepared for the cancelled 1.7.2 candidate. The global-init and environment-root fixes are already merged into main and included in public 1.8.0b1. No browser product changes. Release verification and forward-integration ledger: #1390.)
